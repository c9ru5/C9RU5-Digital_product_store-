<div class="detail-content">
    <div class="container">
        <div class="row mt-5">
            <div class="col-5">
                <img src="<?= _WEB_ROOT ?>/public/assets/images/<?= $detail_product['image'] ?>" class="img-fluid rounded-3" alt="">
            </div>
            <div class="col-7">
                <p>Sản phẩm</p>
                <h3><?= $detail_product['name'] ?></h3>
                <p><i class="bi bi-box2"></i> Tình trạng:
                    <?= $detail_product['quantity'] > 0 ? '<span class="text-success">Còn hàng</span>' : '<span class="text-danger">Hết hàng</span>'; ?>
                    <!-- <span class="text-danger">Hết hàng</span> -->
                </p>
                <p><i class="bi bi-tag"></i> Thể loại: <?= $detail_product['genre'] ?></p>
                <div class="d-flex align-items-center">
                    <h3 class="text-danger" style="font-weight: bolder;"><?= number_format($detail_product['price'] - (($detail_product['price'] * $detail_product['discount_percent']) / 100)) ?>đ</h3>
                    <del style="margin-left: 10px;"><?= number_format($detail_product['price']) ?>đ</del>
                    <span
                        style="background: red; color: white; border-radius: 5px; padding: 5px;  margin-left: 10px;">-<?= $detail_product['discount_percent'] ?>%</span>
                </div>
                <?php if ($detail_product['quantity']): ?>
                ...
                <?php else: ?>
                    <div class="d-flex mt-3">
                        <button type="button" class="btn btn-success mx-2" style="width: 30%;" disabled><i
                                class="bi bi-credit-card-fill"></i> Mua ngay</button>
                        <button type="button" class="btn text-success mx-2"
    ...
    <?php if (!empty($related_product)): ?>
        <div class="container mb-5">
            <div style="display: flex; justify-content: space-between;">
                ...
                <div>
                    <a href="<?= _WEB_ROOT ?>/cua-hang/<?= $detail_product['category_id'] ?>" class="btn btn-success">Xem thêm</a>
                </div>
            </div>
                        ...
                        <a class="text-decoration-none text-dark d-flex"
                            ...
                            <div class="card border-0 bg-transparent mt-0 mx-auto"
                                ...
                                <div class="card-body px-0">
                                    <p class="card-title text-truncate" title="<?= $product['name'] ?>" style="font-weight: bolder;"><?= $product['name'] ?></p>
